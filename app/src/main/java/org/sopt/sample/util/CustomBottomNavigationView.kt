...
class CustomBottomNavigationView : BottomNavigationView {
    ...
    private fun init() {
        ...
        mPaint.color = ContextCompat.getColor(context, R.color.black1_000000) // 네비게이션 색상
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        mFirstCurveStartPoint.set( // CURVE_CIRCLE_RADIUS 굴곡담당
            (mNavigationBarWidth / 1.85 - CURVE_CIRCLE_RADIUS * 2 - CURVE_CIRCLE_RADIUS / 3).toInt(),
            0
        )
        ...
        mSecondCurveEndPoint.set(
            (mNavigationBarWidth / 2.15 + CURVE_CIRCLE_RADIUS * 2 + CURVE_CIRCLE_RADIUS / 3).toInt(),
            0
        )
